...
string BingMapsRasterModel::tileUrl(const std::string& layer, Zoom z, const Kompas::Core::TileCoords& coords) const {
    ...
    unsigned int remainingX = coords.x;
    unsigned int remainingY = coords.y;
    for(Zoom i = z; i != 0; --i) {
        unsigned int cX = remainingX/pow2(i-1);
        unsigned int cY = remainingY/pow2(i-1);
        tilenum << cY*2 + cX;
        remainingX -= cX*pow2(i-1);
        remainingY -= cY*pow2(i-1);
    }
